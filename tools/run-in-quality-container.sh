...
project_label='org.rptadvanced.test.project=librptadvradio'
scope_label="org.rptadvanced.test.scope=$scope"
name="librptadvradio-test-$scope-$$"
pull_image=${RPTADV_CONTAINER_PULL:-1}

## @brief Remove only stopped test containers belonging to this workspace.
cleanup_stale()
{
	...
	while IFS= read -r container; do
		...
		# Never kill a parallel check, even if a stopped container was restarted
		# after discovery. A concurrent cleanup may also have removed it already.
