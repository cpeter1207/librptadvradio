...
C_CONFIG_PARSER_SMOKE_SOURCE := tests/config_parse_smoke.c
C_CONFIG_PARSER_SMOKE_BINARY := build/config-parse-smoke
...
DEBIAN_RUNTIME_DEB = $(DEBIAN_OUTPUT_DIR)/librptadvradio1_$(DEBIAN_VERSION)_$(DEBIAN_ARCH).deb
...
static-analysis:
	...
	$(CPPCHECK) --force --enable=warning,style,performance,portability \
		--error-exitcode=1 --std=c11 -Iinclude $(C_SMOKE_SOURCE) $(C_CONFIG_PARSER_SMOKE_SOURCE)
...
test:
	$(PYTHON) -m unittest discover -s tests -p 'test_container_launcher.py'
	$(CARGO) test --all-targets --locked
	$(MAKE) $(C_SMOKE_BINARY) $(C_CONFIG_PARSER_SMOKE_BINARY)
	./$(C_SMOKE_BINARY)
	./$(C_CONFIG_PARSER_SMOKE_BINARY)
...
$(C_CONFIG_PARSER_SMOKE_BINARY): $(C_CONFIG_PARSER_SMOKE_SOURCE) $(HEADER) $(LIBRARY_LINK) | build
	$(CC) -std=c11 -Wall -Wextra -Werror -Iinclude $< -Lbuild \
		-l$(CRATE) -Wl,-rpath,'$$ORIGIN' -o $@
...
dist: | build
	...
	tar --exclude=.git --exclude=.work --exclude=build --exclude=target --exclude=__pycache__ \
		...
		--exclude=debian/librptadvradio1 --exclude=debian/librptadvradio-dev \
...
distcheck: dist
	! tar -tzf build/$(PACKAGE)-$(PACKAGE_VERSION).tar.gz | \
		grep -E '/debian/(\.debhelper/|debhelper-build-stamp$$|files$$|tmp/|librptadvradio(1|-dev)/|.*\.(substvars|debhelper\.log)$$)'
